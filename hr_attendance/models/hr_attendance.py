...
class HrAttendance(models.Model):
    ...
    def _check_validity(self):
        ...
        for attendance in self:
            ...
            if last_attendance_before_check_in and last_attendance_before_check_in.check_out and last_attendance_before_check_in.check_out > attendance.check_in:
                last_attendance_before_check_in.unlink()
            ...
            if not attendance.check_out:
                ...
                if last_attendance_before_check_out and last_attendance_before_check_in != last_attendance_before_check_out:
                    last_attendance_before_check_out.unlink()
                    # raise exceptions.ValidationError(_("Cannot create new attendance record for %(empl_name)s, the employee was already checked in on %(datetime)s") % {
                    #     'empl_name': attendance.employee_id.name,
    ...
    def _get_attendances_dates(self):
        ...
        for attendance in self.filtered(lambda a: a.employee_id.company_id.hr_attendance_overtime and a.check_in):
            ...
            if check_in_day_start[0] < datetime.combine(attendance.employee_id.company_id.overtime_start_date,
                                                        datetime.min.time()):
                continue
